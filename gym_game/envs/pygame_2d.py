...
'''
The "Soldier" class serves as the designated class for both players, encompassing the necessary functions to facilitate 
their movement, shooting capabilities, and the ability to determine their respective statuses of being alive or deceased. 
This class incorporates a comprehensive set of functionalities to enable smooth gameplay interactions, 
ensuring that players can perform essential actions, engage in combat, and assess their current state within the game.
'''
...
'''
The "HealthBar" class is specifically designed to manage the health status of the players. 
Its primary purpose is to track and update the current health levels of the players throughout the game. 
When a player is hit, the HealthBar class is responsible for reflecting the changes in health by updating 
the corresponding visual representation on the surface. By utilizing this class, the game can effectively 
monitor and display the real-time health status of each player, allowing for dynamic and visually informative 
...
'''
Each bullet in the game is represented as an instance of the "Bullet" class. 
These instances are added to a "bullet_group" container to facilitate tracking their current positions 
and monitor their interactions with other game entities. The "bullet_group" allows for efficient management 
and easy access to each bullet's attributes, such as position and velocity.
'''
